...
// nowFunc is used for time in the recorder; tests can override for deterministic behavior.
...
// NewConsumptionLagRecorder creates a lag recorder. consumerGroup must be non-empty
// (after trimming whitespace). If maxMins > 0, only the last maxMins minute buckets
// are retained. consumerGroup is included in emitted MinuteLagMetric. If minuteLagChan
// is non-nil, a MinuteLagMetric is sent on it on each minute rollover (non-blocking send).
func NewConsumptionLagRecorder(maxMins int, consumerGroup string, minuteLagChan chan<- MinuteLagMetric) *ConsumptionLagRecorder {
	if strings.TrimSpace(consumerGroup) == "" {
		panic("consumerGroup must be non-empty")
	}
	return &ConsumptionLagRecorder{
...
func (c *ConsumptionLagRecorder) RecordLag(recTimestamp time.Time, recTopic string, recPartition int32) int64 {
	now := nowFunc()
